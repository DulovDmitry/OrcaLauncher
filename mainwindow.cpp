    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(this, SIGNAL(addTasksToQueueSignal(Queue)),
            infodialog, SLOT(addTasksToQueue(Queue)));
...
void MainWindow::processFileList(QStringList loadedFileNames)
{
    ...
    for (; fileCounter < queue.fileList.size(); ++fileCounter)                             // главный цикл
    {
        ...
        queue.fileThread.append(QString::number(getThreadsNumberFromText(text)));                                                              // записываем в список информацию о количестве потоков, требуемом для отдельного файла
...
{
    ui->plainTextEdit->setPlainText(queue.fileBodies.at(row));                                            // перемещаем текст выбранного файла в окно редактора
    refreshTableStyle();
...
{
    emit addTasksToQueueSignal(queue);
    this->showMinimized();
    ui->plainTextEdit->clear();
}
